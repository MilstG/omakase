...
The app is organized into five tabs, each labeled with a kanji that names its role.
...
### 5. Costeo de menú (原価) — recipe costing

Builds the **theoretical food cost** bottom-up: ingredients → per-course escandallo → cost per cover.

- **Insumos** are entered in **ARS, IVA-included** (as invoices arrive), with their IVA rate, purchase quantity, and **yield %** (rendimiento — the fraction that reaches the plate after trimming/merma). The app derives **USD per usable unit** using the exchange rate. Prices unedited for 30+ days are flagged.
- **Pasos** are the omakase sequence: each course is a mini-recipe (ingredient × quantity per portion), reorderable, with a standard/premium variant toggle.
- The **standard menu price is derived live from the cost model's turno price** (net of IVA), so the two tabs can never diverge; the premium price is set locally.
- Outputs: cost per cover, theoretical food cost % vs. the plan % in the cost model, a Pareto of ingredient cost contribution, a single-ingredient price-shock simulator, and a reverse-calculated suggested price for a target food cost.
- **Enviar al esquema** overwrites the cost model's food-cost % with the theoretical value plus a configurable cushion (pp) for operational waste; the full escandallo exports to CSV.

Together with the other tabs this closes a reconciliation triangle: **theoretical** (recipes) vs. **plan** (勘定) vs. **actual** (番付). If actual CMV runs well above theoretical, the gap is operational (waste, portioning, purchase prices) — not the recipe.
...
| `kanjo:genka` | Menu costing: insumos, pasos, and parameters (Costeo tab) |
